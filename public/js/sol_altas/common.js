...
$(document).ready(function () {
    ...
    //Ejecuta el onchange de el combo para los div no visibles
    ...
    $("#plan_celular_select").combobox({
        select: function (event, ui) {
            ...
            if ((servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV") && (esquemas == 2 || esquemas == 6)) {
                costoVenta();
            }
        }
    });
...
$("#codigo_auto,#codigo_celular, #codigo_computadora, #codigo_software, #codigo_bam, #codigo_poliza, #codigo_sueldo").keyup(function () {
    ...
    switch (id) {
        case 'codigo_auto':
            link = "get_codigos_auto";
            break;
        case 'codigo_celular':
            link = "get_codigos_celular";
            break;
        case 'codigo_computadora':
            link = "get_codigos_computadora";
            break;
        case 'codigo_software':
            link = "get_codigos_software";
            break;
        case 'codigo_bam':
            link = "get_codigos_bam";
            break;
        case 'codigo_poliza':
            link = "get_codigos_poliza";
            break;
        case 'codigo_sueldo':
            link = "get_codigos_sueldo";
            break;
    }
...
$("#guarda_info").click(function () {
    ...
    if (cliente == "NOK" && (servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV")) {
        tipo_contratacion = 2;//NOK-RREC
    } else if (cliente != "NOK" && (servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV")) {
...
$("#actualiza_info").click(function () {
    ...
    if (servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV") { //EVALUACION PARA NOK-RREC
        console.log("si entra aca");
        $("#formulario_solicitud_update input").each(function () {
            if ($(this).hasClass('req_nok')) {
                if ($(this).val() == "") {
    ...
    } else if (cliente != "NOK" && (servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV")) { // EVALUACION PARA RREC (RAD, por ejemplo)
        $("#formulario_solicitud_update input").each(function () {
            if ($(this).hasClass('req_rrec')) {
                if ($(this).val() == "") {
                    $(this).css("border", "1px solid red");
...
function muestraCodigos(id, valor) {
    ...
    if (valor) {
        ...
        if ((servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV") && id != "correo" && (esquemas == 3 || esquemas == 4)) {
    ...
    if (servicio == "RREC" || servicio === "POLZ" || servicio === "TKBS" || servicio === "SERV") {
        costoVenta();
...
function calcularCostoRecurso() {
    ...
    $.post("/pet_sol/calcular_costos_recuros", {
        riesgo: riesgo,
        imss: imss,
        variable: variable,
        esquema: esquema,
